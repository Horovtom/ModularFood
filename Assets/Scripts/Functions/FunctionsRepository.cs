...
public class FunctionsRepository {
	...
	private Vector3 CubeShape(float u, float v) {
		...
		x = Mathf.Cos(u) * Mathf.Cos(v) * Mathf.Pow((Mathf.Pow(Mathf.Abs(Mathf.Cos(4f * u / 4f)), 100f) + Mathf.Pow(Mathf.Abs(Mathf.Sin(4f * u / 4f)), 100f)), (-1f / 100f)) * Mathf.Pow((Mathf.Pow(Mathf.Abs(Mathf.Cos(4f * v / 4f)), 100f) + Mathf.Pow(Mathf.Abs(Mathf.Sin(4f * v / 4f)), 100f)), (-1f / 100f));
		y = Mathf.Cos(u) * Mathf.Sin(v) *
	...
	private Vector3 CustomShape(float u, float v) {
		...
		if (changed) {
			changed = false;
			vRange = new Vector2(-(Mathf.PI / 2f), Mathf.PI / 2f);
			uRange = new Vector2(0, Mathf.PI * 2);
		}
